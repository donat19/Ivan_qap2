...
function snake(value) {
  ...
  function formatCoords(...values) {
    let validCoords = values.map(value => {
        try {
  ...
  function mimeFromFilename(filename) {
    ...
    const mimeTypes = {
        ...
        'ico': 'image/x-icon', 'cur': 'image/x-icon', 'png': 'image/png', 'svg': 'image/svg+xml',
        'webp': 'image/webp', 'mp3': 'audio/mp3', 'wav': 'audio/wav', 'mp4': 'video/mp4',
        'webm': 'video/webm', 'json': 'application/json', 'mpeg': 'video/mpeg', 'csv': 'text/csv',
        'ttf': 'font/ttf', 'woff': 'font/woff', 'zip': 'application/zip', 'avi': 'video/x-msvideo'
    };
  ...
  function generateLicenseLink(licenseCode, targetBlank = false) {
    ...
    let text = licenses[licenseCode] || "All Rights Reserved";
    let url = licenseCode.startsWith("CC-") ? `https://creativecommons.org/licenses/${licenseCode.replace("CC-", "").toLowerCase()}/4.0/` : "https://choosealicense.com/no-permission/";
  ...
  function pureBool(value) {
    ...
    let truthy = ["yes", "y", "oui", "o", "t", "true", "vrai", "v", "1"];
    let falsy = ["no", "n", "non", "f", "false", "faux", "0", "-1", "-2"];
  ...
  function every(...values) {
    return values.every(val => {
        try { return pureBool(val); } catch { return false; }
    });
}
...
function none(...values) {
  ...
  function buildUrl(query, order, count, license) {
    ...
    if (!["ascending", "descending"].includes(order)) throw new Error("Invalid order");
    if (typeof count !== 'number' || count < 1 || count > 50) throw new Error("Invalid count");
    let validLicenses = ["none", "any", "cc-by", "cc-by-nc", "cc-by-sa", "cc-by-nd", "cc-by-nc-sa", "cc-by-nc-nd"];
    if (!validLicenses.includes(license)) throw new Error("Invalid license");
...
function testPureBool() {
    try {
        let value = document.getElementById("boolInput").value;
        showResult(pureBool(value));
    } catch (error) {
...
function testEvery() {
    let values = document.getElementById("everyInput").value.split(",").map(v => v.trim());
    showResult(every(...values));
}

function testAny() {
    let values = document.getElementById("anyInput").value.split(",").map(v => v.trim());
    showResult(any(...values));
}

function testNone() {
    let values = document.getElementById("noneInput").value.split(",").map(v => v.trim());
    showResult(none(...values));
}
...
function testBuildUrl() {
    try {
        ...
        let url = buildUrl(query, order, count, license);
        showResult(url);
    } catch (error) {
        showResult(error.message);
    }
